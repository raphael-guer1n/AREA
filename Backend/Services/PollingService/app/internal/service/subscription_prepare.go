...
func (s *SubscriptionService) applyPrepareSteps(userID int, providerConfig *config.PollingProviderConfig, cfg map[string]any) (map[string]any, error) {
...
func matchesPrepareCondition(cond *config.PollingPrepareCondition, cfg map[string]any) (bool, error) {
	if cond == nil {
		return true, nil
	}
...
func (s *SubscriptionService) applyFetchStep(userID int, provider string, fetch *config.PollingProviderFetchConfig, cfg map[string]any) error {
	if fetch == nil {
		return nil
	}
	if s.requestSvc == nil {
		return fmt.Errorf("%w: request service not configured", ErrInvalidConfig)
	}
	...
	for {
		...
		responseBody, err := s.requestSvc.ExecuteRequest(request, provider, userID, ctx, queryOverrides)
		if err != nil {
			return err
		}
...
func applyTemplateListStep(step *config.PollingProviderTemplateListConfig, cfg map[string]any) error {
	if step == nil {
		return nil
	}
...
func applyExtractStep(step *config.PollingProviderExtractConfig, cfg map[string]any) error {
	if step == nil {
		return nil
	}
...
func applyGenerateStep(step *config.PollingProviderGenerateConfig, cfg map[string]any) error {
	if step == nil {
		return nil
	}
